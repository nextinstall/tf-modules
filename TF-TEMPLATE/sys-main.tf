data "template_file" "win_bootstrap" {
  ...
  vars {
    system_name = "${var.env_level}-${var.product}-${var.role}${format("%1d", (count.index))}"
  }
}
...
resource "aws_instance" "instance" {
  ...
  volume_tags {
    Name         = "${var.env_level}-${var.product}-${format("WEB%1d", count.index)}-EBS-primary"
  ...
  tags {
    Name         = "${var.env_level}-${var.product}-${format("WEB%1d", count.index)}"
    Environment  = "${var.Environment}"
    env_level    = "${var.env_level}"
    platform     = "${var.platform}"
    product      = "${var.product}"
...
resource "aws_ebs_volume" "instance" {
  ...
  tags {
    Name         = "${var.env_level}-${var.product}-${format("WEB%1d", count.index)}-EBS-secondary"
    Environment  = "${var.Environment}"
    env_level    = "${var.env_level}"
    platform     = "${var.platform}"
    product      = "${var.product}"
